...
type WorkItem = {
  ...
  visual_bg?: string;
  visual_word?: string;
  visual_image?: string;  // Image for hero visual
  frame1_label?: string;
  frame1_word?: string;
  frame1_bg?: string;
  frame1_image?: string;  // Image instead of gradient
  frame2_label?: string;
  frame2_word?: string;
  frame2_bg?: string;
  frame2_image?: string;  // Image instead of gradient
...
export default function WorkPanel() {
  ...
  const [selectingImageFor, setSelectingImageFor] = useState<'featured' | 'gallery' | 'frame1' | 'frame2' | 'visual'>('featured');
  ...
  const openMediaSelector = (type: 'featured' | 'gallery' | 'frame1' | 'frame2' | 'visual') => {
  ...
  const selectImage = (media: MediaItem) => {
    ...
    } else if (selectingImageFor === 'gallery') {
      setEditingItem({
        ...editingItem,
        gallery_images: [...editingItem.gallery_images, media.path]
      });
    } else if (selectingImageFor === 'frame1') {
      setEditingItem({ ...editingItem, frame1_image: media.path });
    } else if (selectingImageFor === 'frame2') {
      setEditingItem({ ...editingItem, frame2_image: media.path });
    } else if (selectingImageFor === 'visual') {
      setEditingItem({ ...editingItem, visual_image: media.path });
    }
          ...
          <div style={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={styles.tabs}>
              ...
              <button
                ...
                style={{
                  ...styles.tab,
                  borderBottom: activeTab === 'design' ? '2px solid #2bb6ea' : '2px solid transparent',
                  color: activeTab === 'design' ? '#2bb6ea' : 'rgba(246,245,240,0.5)'
                }}
              >
                🎨 Visual Frames
              </button>
            ...
            <div style={{ display: 'grid', gap: '20px' }}>
              ...
              <>
                <div>
                  <label style={styles.label}>Challenge / The Brief</label>
                  <textarea
                ...
                <div>
                  <label style={styles.label}>Solution / What We Did</label>
                  <textarea
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                  />
                  <div style={{ color: 'rgba(246,245,240,0.4)', fontSize: '12px', marginTop: '6px' }}>
                    💡 Tip: Start with number+symbol for stats (e.g., "+212%", "4.2×", "38%")
                  </div>
                </div>

                {/* Testimonial */}
                <div style={{ background: 'rgba(246,245,240,0.03)', padding: '16px', borderRadius: '8px', marginTop: '12px' }}>
              ...
              {/* DESIGN TAB - VISUAL FRAMES */}
              {activeTab === 'design' && (
              <>
                <div style={{ color: 'rgba(246,245,240,0.6)', fontSize: '13px', padding: '16px', background: 'rgba(43,182,234,0.05)', borderRadius: '8px' }}>
                  🎨 Upload images for the 3 big banner sections on your detail page (Hero + 2 Frames). Each banner will be full-screen height.
                </div>
                ...
                <div style={{ border: '1px solid rgba(246,245,240,0.1)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ color: '#f6f5f0', fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>
                    🖼️ Hero Visual (Top banner - first thing visitors see)
                  </div>
                  
                  <div style={{ display: 'grid', gap: '12px' }}>
                    {/* Hero Image */}
                    <div>
                      <label style={styles.label}>Hero Background Image (recommended)</label>
                      {editingItem.visual_image ? (
                        <div style={{ position: 'relative', marginBottom: '8px' }}>
                          <img src={editingItem.visual_image} alt="Hero Visual" style={{ width: '100%', height: '120px', objectFit: 'cover', borderRadius: '8px' }} />
                          <button
                            onClick={() => updateField('visual_image', '')}
                            style={{
                              position: 'absolute',
                              top: '8px',
                              right: '8px',
                      ...
                      <button onClick={() => openMediaSelector('visual')} style={{ ...styles.btnSecondary, width: '100%' }}>
                        {editingItem.visual_image ? 'Change Image' : '+ Upload Hero Image'}
                      </button>
                      <div style={{ color: 'rgba(246,245,240,0.4)', fontSize: '11px', marginTop: '6px' }}>
                        💡 This appears at the top of your detail page - make it impactful!
                      </div>
                    </div>

                    <div>
                      <label style={styles.label}>Visual Word/Text (large, centered)</label>
                      <input
                        type="text"
                        value={editingItem.visual_word || ''}
                        onChange={(e) => updateField('visual_word', e.target.value)}
                        placeholder="e.g., Client name or project word"
                        style={styles.input}
                      />
                    </div>

                    <div>
                      <label style={styles.label}>Background Gradient (fallback)</label>
                      <input
                        type="text"
                        value={editingItem.visual_bg || ''}
                        onChange={(e) => updateField('visual_bg', e.target.value)}
                        placeholder="e.g., linear-gradient(135deg, #1a1f5c 0%, #2bb6ea 100%)"
                        style={styles.input}
                      />
                      <div style={{ 
                        marginTop: '8px', 
                        ...
                        borderRadius: '8px',
                        ...(editingItem.visual_image 
                          ? { backgroundImage: `url(${editingItem.visual_image})`, backgroundSize: 'cover', backgroundPosition: 'center' }
                          : { background: editingItem.visual_bg || 'rgba(246,245,240,0.05)' }
                        ),
                        border: '1px solid rgba(246,245,240,0.1)',
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        {editingItem.visual_word || editingItem.client || 'Preview'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ border: '1px solid rgba(246,245,240,0.1)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ color: '#f6f5f0', fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>
                    🖼️ Frame 1 (Appears after "The Brief" section)
                  </div>
                  
                  <div style={{ display: 'grid', gap: '12px' }}>
                    {/* Frame 1 Image */}
                    <div>
                      <label style={styles.label}>Background Image (recommended)</label>
                      {editingItem.frame1_image ? (
                        <div style={{ position: 'relative', marginBottom: '8px' }}>
                          <img src={editingItem.frame1_image} alt="Frame 1" style={{ width: '100%', height: '120px', objectFit: 'cover', borderRadius: '8px' }} />
                          <button
                            onClick={() => updateField('frame1_image', '')}
                            style={{
                              position: 'absolute',
                              top: '8px',
                              right: '8px',
                      ...
                      <button onClick={() => openMediaSelector('frame1')} style={{ ...styles.btnSecondary, width: '100%' }}>
                        {editingItem.frame1_image ? 'Change Image' : '+ Upload Frame Image'}
                      </button>
                      <div style={{ color: 'rgba(246,245,240,0.4)', fontSize: '11px', marginTop: '6px' }}>
                        💡 Image will be used as background. Gradient below is fallback if no image.
                      </div>
                    </div>

                    <div>
                      <label style={styles.label}>Small Label (top-left corner)</label>
                      <input
                        type="text"
                        value={editingItem.frame1_label || ''}
                        onChange={(e) => updateField('frame1_label', e.target.value)}
                        placeholder="e.g., VISUAL — BRAND"
                        style={styles.input}
                      />
                    </div>
                    <div>
                      <label style={styles.label}>Main Word/Text (large, centered)</label>
                      <input
                        type="text"
                        value={editingItem.frame1_word || ''}
                        onChange={(e) => updateField('frame1_word', e.target.value)}
                        placeholder="e.g., Ummah"
                        style={styles.input}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingItem.frame1_bg || ''}
                        onChange={(e) => updateField('frame1_bg', e.target.value)}
                        placeholder="e.g., linear-gradient(135deg, #0a4d3a 0%, #1a8a65 100%)"
                        style={styles.input}
                      />
                      <div style={{ 
                        marginTop: '8px', 
                        ...
                        borderRadius: '8px',
                        ...(editingItem.frame1_image 
                          ? { backgroundImage: `url(${editingItem.frame1_image})`, backgroundSize: 'cover', backgroundPosition: 'center' }
                          : { background: editingItem.frame1_bg || 'rgba(246,245,240,0.05)' }
                        ),
                        border: '1px solid rgba(246,245,240,0.1)',
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        {editingItem.frame1_word || 'Preview'}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ border: '1px solid rgba(246,245,240,0.1)', borderRadius: '12px', padding: '20px' }}>
                  <div style={{ color: '#f6f5f0', fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>
                    🖼️ Frame 2 (Appears after "Solution" section)
                  </div>
                  ...
                  <div style={{ display: 'grid', gap: '12px' }}>
                    ...
                    <div>
                      <label style={styles.label}>Background Image (recommended)</label>
                      {editingItem.frame2_image ? (
                        <div style={{ position: 'relative', marginBottom: '8px' }}>
                          <img src={editingItem.frame2_image} alt="Frame 2" style={{ width: '100%', height: '120px', objectFit: 'cover', borderRadius: '8px' }} />
                          <button
                            onClick={() => updateField('frame2_image', '')}
                            style={{
                              position: 'absolute',
                              top: '8px',
                              right: '8px',
                      ...
                      <button onClick={() => openMediaSelector('frame2')} style={{ ...styles.btnSecondary, width: '100%' }}>
                        {editingItem.frame2_image ? 'Change Image' : '+ Upload Frame Image'}
                      </button>
                      <div style={{ color: 'rgba(246,245,240,0.4)', fontSize: '11px', marginTop: '6px' }}>
                        💡 Image will be used as background. Gradient below is fallback if no image.
                      </div>
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingItem.frame2_label || ''}
                        onChange={(e) => updateField('frame2_label', e.target.value)}
                        placeholder="e.g., PLATFORM — EXPERIENCE"
                        style={styles.input}
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingItem.frame2_word || ''}
                        onChange={(e) => updateField('frame2_word', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="text"
                        value={editingItem.frame2_bg || ''}
                        onChange={(e) => updateField('frame2_bg', e.target.value)}
                        placeholder="e.g., linear-gradient(135deg, #f4d03f 0%, #e8a93f 100%)"
                        style={styles.input}
                      />
                      <div style={{ 
                        marginTop: '8px', 
                        ...
                        borderRadius: '8px',
                        ...(editingItem.frame2_image 
                          ? { backgroundImage: `url(${editingItem.frame2_image})`, backgroundSize: 'cover', backgroundPosition: 'center' }
                          : { background: editingItem.frame2_bg || 'rgba(246,245,240,0.05)' }
                        ),
                        border: '1px solid rgba(246,245,240,0.1)',
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        {editingItem.frame2_word || 'Preview'}
                      </div>
                    </div>
                  </div>
                </div>

                {/* Gallery Images (moved to design tab) */}
                <div style={{ border: '1px solid rgba(246,245,240,0.1)', borderRadius: '12px', padding: '20px', marginTop: '12px' }}>
                  <div style={{ color: '#f6f5f0', fontSize: '16px', fontWeight: 600, marginBottom: '8px' }}>
                    📷 Gallery Images
                  </div>
                  <div style={{ color: 'rgba(246,245,240,0.5)', fontSize: '12px', marginBottom: '16px' }}>
                    Additional project images shown in a gallery on the detail page
                  </div>
